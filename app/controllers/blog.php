                ...
                if ($UserBlogID == null) {
                    ...
                    if(isset($argv[1])) {
                        ...
                        if ($post_info == null) {
                            ...
                            Router::view(['page'=>'post','post_info' => $post_info, 'blog_name'=>$loginid]);
                        }
                    }else {
                        $blog_info = BlogController::getBlog($UserBlogID);
                        $blog_by_page = BlogController::getBlogbyPageX($blog_info);
                        if(!isset($blog_by_page['row'])){
                            Router::view(['page'=>'blog','blog_name'=>$loginid,'blog_max_page'=>$blog_by_page['max_page']]);
                        }else{
                        Router::view(['page'=>'blog','blog_info' => $blog_by_page['row'],'blog_current_page'=>$blog_by_page['cur_page'], 'blog_max_page'=> $blog_by_page['max_page'], 'blog_name' => $loginid]);
                        }
                    }
                }
            }else{
                Router::view(['page'=>'404']); //e.g /u/test/2/randomstring
            }
        }
    }
